...
const MiniCalendar = ({ date }) => {
  ...
  for (let i = 0; i < 42; i += 7) { 
    if (i >= 28 && days[i] === '' && rows.length >= 4) break; 
 ...
 */
const CalendarGrid = ({ currentDate, events, selectedEventIds, onEventClick }) => {
  ...
  // 1. Padding days at the start of the month
  ...
  const totalCellsNeeded = blanks.length + activeDays.length;
    ...
    <div className="print-container">
      ...
      <div className={`calendar-grid rows-${rowCount}`}>
        ...
        {allSlots.map((slot, index) => {
          ...
          return (
            <div key={index} className={`calendar-day ${isE ? 'empty-day' : ''}`}>
              ...
              {isE && slot.isPrev && <MiniCalendar date={prevMonthDate} />}
              {isE && slot.isNext && <MiniCalendar date={nextMonthDate} />}
            </div>
          );
        })}
      </div>
